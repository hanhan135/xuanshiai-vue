...
> 请在 `xuanshiai-vue` 项目目录操作，不依赖某台电脑的绝对路径。更新日期：2026-08-21。
...
2. 若提示“是否转换为 uni-app 项目 / 关联云服务空间”，开发期可跳过或取消关联；当前仓库默认 `USE_MOCK = false`、`API_BASE_URL=https://xhztest.xyz`，不依赖 uniCloud。本机 FastAPI 仅作可选本地联调。
...
1. 启动页为首页，非白屏、非持续报错。
2. 底部 Tab：首页 / 牵线 / 社区 / 消息 / 我的 均可切换。
3. 首页推荐区出现人物故事，或显示“认真为你匹配中… / 暂无推荐”。
4. 点“申请认识”能弹出说明与附言。
5. 社区列表能出动态（关注/同城/发现可切换）；不应长期停在“网络异常”。
6. 牵线、消息列表至少有加载态或列表内容。
7. 控制台无阻断级红错；仓库默认 `USE_MOCK = false` 时走测试服 `https://xhztest.xyz`。本机联调可临时改为 `http://127.0.0.1:8000`。

#### 本机网络地址

仓库默认 `API_BASE_URL=https://xhztest.xyz`。本机 FastAPI 联调时临时改为 `http://127.0.0.1:8000`；真机访问再用 `LAN_API_BASE_URL`，并仅向本地子网放行 TCP 8000。
